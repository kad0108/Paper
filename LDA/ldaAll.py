...
if __name__=='__main__':
    ...
    #     doc += item['content']
    # docfile = open('./data/docAll.txt', 'w')
    # docfile.write(doc)
    # docfile.close()
    # read from mongo finished
    
    doc = open('./data/docAll.txt').read()
    segs = jieba.cut(doc)
    segs_list = list(segs)
    ...
    resultfile = open('./data/all.txt', 'w');
    resultfile.write(' '.join(final_list))
    resultfile.close()

    print "OK"

    wordmapfile  = './model/all/wordmap.txt'
    trnfile = "./data/all.txt"
    modelfile_suffix = "./model/all/final"
